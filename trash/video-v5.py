    ...
    def update_video_frame(self):
        if self.mode == "VIDEO" and self.cap:
            ...
            if ret:
                ...
                glBindTexture(GL_TEXTURE_2D, self.texid)
                # Faster than glTexImage2D for streaming
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, data)

    def update_node(self, idx, gx, gy):
        if self.mode == "IMAGE": # Lock warping in video mode
...
def main():
    ...
    while running:
        ...
        if show_ui:
            ...
            text_renderer.draw(f"Current Mode: {warper.mode}", 40, y_base)
            text_renderer.draw(f"[P] Toggle Video/Image", 40, y_base + 35)
        ...
        pygame.display.flip()
        clock.tick(30) # 30 FPS for stable video playback on Pi 4
